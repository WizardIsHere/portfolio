    ...
    useGSAP(() => {
        ...
        const handleMouseMove = (e) => {
            ...
            icons.forEach((icon) => {
                ...
                const scale = 1 + Math.exp(-(distance ** 2) / 2000) * 0.6;
                gsap.to(icon, {
                    scale: scale,
                    duration: 0.2,
                    overwrite: 'auto',
            ...
            >
                ...
                {dockApps.map(({ id, name, icon, canOpen }) => (
                    <div key={id} className='dock-icon-item relative group flex flex-col items-center'>
                        ...
                        {canOpen && <div className="w-1 h-0 bg-white/60 rounded-full mt-0 opacity-0 group-hover:opacity-100 transition-opacity" />}
                    </div>
                ))}
            </div>
        </section>
